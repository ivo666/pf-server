    ...
    for line in data.split('\n'):
        if line.strip() and not line.startswith(('Date', 'Total')):
            ...
            if len(parts) >= 6:  # Убедитесь, что есть все необходимые поля
                ...
                cost = float(parts[6])  # Форматируем стоимость
...
def get_data_from_yandex_direct(start_date):
...
if __name__ == "__main__":
    ...
    try:
        ...
        while current_date <= datetime.now():
            ...
            data = get_data_from_yandex_direct(current_date.strftime("%Y-%m-%d"), week_end_date.strftime("%Y-%m-%d"))
